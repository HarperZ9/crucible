...
spec. Whether the verdict card is TRUE of src/crucible/verdict.py is a
different question, and tests/test_verdict_card.py settles that one by driving
the function.
"""
...
def test_the_seed_is_recorded_on_the_mark():
    ...
    svg = R.header_svg({"name": "crucible", "role": "x", "tagline": "y",
                        "words": []})
    assert f"SEED {R.seed_for('crucible') % 100000:05d}" in svg
...
def _card_text_that_overflows(card):
    ...
    # character count is a fair proxy. The note column is proportional, so it
    # is measured with the renderer's own width table instead.
    ...
    for field in card["fields"]:
        ...
        bad += _lines_that_run_long(field["note"], f'the note under '
                                    f'{field["key"]!r}', CARD.NOTE_BUDGET,
                                    CARD.NOTE_LINES)
...
def _lines_that_run_long(text, label, budget, limit):
    """Two ways a wrapped column goes wrong, and the second is the one the
    fleet's gates miss. Dropping the ending is the obvious one. The other is a
    single token longer than the budget: the wrapper is greedy, so it leaves
    that token alone on its line, the joined text still equals the source, and
    the drawing runs off the page with every check green."""
...
def test_that_card_check_can_actually_fail():
    """A green suite otherwise proves only that the check ran. The third row
    is the greedy-wrap case: one unbreakable token, nothing dropped."""
    control = {
...
# prose. Their presence is covered by tests/test_readiness.py.
SHOWN_DIRS = ("docs/art",)
